...
public class UIUtilities {
    ...
    static func makeSquareGrayImage(image: UIImage) -> UIImage {
        if image.size.width > image.size.height {
            ...
            // 회색 이미지 만들기
            let color = UIColor.gray
            let sizes = CGSize(width: image.size.height - image.size.width, height: image.size.height)
            ...
            let size = CGSize(width: image.size.height, height: image.size.height)
            UIGraphicsBeginImageContext(size)
            
            let oriSize = CGRect(x: 0, y: 0, width: image.size.width, height: image.size.height)
            let graySize = CGRect(x: image.size.width, y: 0, width: image.size.height - image.size.width, height: image.size.height)
            ori_img.draw(in: oriSize)
            grayImage.draw(in: graySize)
    ...
    ////        / 앨범 저장 코드
    public static func image_save(cvBuffer: CVImageBuffer) {
        let uiImage = UIUtilities.createUIImage(from: cvBuffer, orientation: UIImage.Orientation.up)
